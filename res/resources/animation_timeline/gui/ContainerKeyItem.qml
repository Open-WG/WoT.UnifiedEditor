			...
			delegate: Item {
				...
				function getKeyWidth() {
					var width = Math.round(
							styleData.timelineController.fromSecondstoScale(
								model.itemKeyPosition + model.itemDuration) - keyHolder.x)
				...
				Rectangle {
					...
					BlendItem {
						...
						width: Math.round(
							styleData.timelineController.fromSecondstoScale(
								model.itemKeyPosition + model.itemKeyBlendInTime)) - keyHolder.x
					}
					...
					BlendItem {
						...
						readonly property real _blendOutPosX: Math.round(
							styleData.timelineController.fromSecondstoScale(
								model.itemKeyPosition + model.itemDuration - model.itemKeyBlendOutTime)) 
								- keyHolder.x
						...
						function handleSelection(mouse) {
							...
							if (ctrlPressed) {
								...
								if (selectionModel.hasSelection) {
									...
									var type = styleData.model.model.get(selectedInd).itemType
					...
					MouseArea {
						...
						enabled: model.itemKeyContainerResizable

						height: parent.height
						width: Constants.seqContainerKeySizerWidth
						z: 15

						cursorShape: model.itemKeyContainerResizable ? Qt.SizeHorCursor : Qt.ArrowCursor
						preventStealing: true
						hoverEnabled: true
						...
						onPositionChanged: {
							...
							var prevItemPos = model.itemKeyPosition
							var finalItemDur = model.itemDuration - (seconds - prevItemPos)

							if (finalItemDur >= minDuration){
							 	model.itemKeyPosition = seconds
								model.itemDuration -= model.itemKeyPosition - prevItemPos
							}
							else {
								model.itemKeyPosition += itemDuration - minDuration
								model.itemDuration = minDuration
							}
						}
					}
					...
					MouseArea {
						...
						onPositionChanged: {
							...
							var finalValue = seconds - model.itemKeyPosition

							if (finalValue >= minDuration){
							 	model.itemDuration = seconds - model.itemKeyPosition
							}
							else {
								model.itemDuration = minDuration
							}
