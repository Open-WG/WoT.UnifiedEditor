...
/**
 * There is a little difference between TextFieldController and SearchFieldController.
 * TODO: Need to consider merging these controllers.
 */
MouseArea {
	id: controller
	...
	SM.StateMachine {
		...
		SM.State {
			id: initState
			onEntered: {
				controller.control.dirty = false
				controller.oldValue = Qt.binding(function() {return controller.control.text})
			}
			onExited: {
				controller.oldValue = controller.oldValue
			}
