...
LodsDelegate {
	...
	Column {
		...
		Details.CameraSlider {
			width: parent.width
			model: delegateRoot.lodbarModel
			to: delegateRoot.lodbarRange
			decimals: 2
			units: Settings.lodExtentUnits

			onModified: {
				if (!delegateRoot.readOnlyCamera) {
					delegateRoot.setCameraDistance(value, !commit)
				}
			}

			Binding on value {
				value: delegateRoot.cameraDistance
			}
		}
	}
}
