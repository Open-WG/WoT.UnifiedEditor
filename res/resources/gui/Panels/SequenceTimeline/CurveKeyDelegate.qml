...
Item {
	...
	function getKeyYPosition() {
		return compositeHolder.height -
			curveEditorController.fromValueToPixels(itemData.value) - height / 2
	}
	...
	TangentControllerItem {
		...
		property bool tangentIsBroken: itemData.isBrokenTangent
		...
		function getRotation(tangent, mode) {
			...
			var xPixel = context.timelineController.fromValueToPixels(
				context.timelineController.visibleStart + dir.x)
	...
	Component {
		...
		CurveKeyContextMenu {
			...
			sequenceModel: context.sequenceModel
	...
	Rectangle {
		...
		color: itemData.curveComponentColor
