...
Rectangle {
	...
	signal removeItems(var selection)

	property Component edgeDelegate: null
	property Component vertexDelegate: null
		...
		Repeater {
			...
			model: DelegateModel {
				...
				delegate: Item {
					...
					property var delegateName: index != -1 ? "details/" + ["Edge", "Vertex"][index] + "Delegate.qml" : ""
	...
	Shortcut {
		...
		onActivated: {
			if (selectionModel)
				removeItems(selectionModel.selection)
		}
	}
}
